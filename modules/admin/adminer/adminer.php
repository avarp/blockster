...
login($fe,$G){global$x;if($x=="-qlite")return
...
<table cellspacing="0">
';$Jf=array();if(DB!=""){$db=($a!=""?"":" checked");echo"<thead><tr>","<th style='text-align: left;'><label class='module'><input type='checkbox' id='check-tables'$db onclick='formCheck(this, /^tables\\[/);'>".lang(115)."</label>","<th style='text-align: right;'><label class='module'>".lang(137)."<input type='checkbox' id='check-data'$db onclick='formCheck(this, /^data\\[/);'></label>","</thead>\n";$oi="";$lh=tables_list();foreach($lh
as$C=>$U){$If=preg_replace('~_.*~','',$C);$db=($a==""||$a==(substr($a,-1)=="%"?"$If%":$C));$Mf="<tr><td>".checkbox("tables[]",$C,$db,$C,"checkboxClick(event, this); formUncheck('check-tables');","module");if($U!==null&&!preg_match('~table~i',$U))$oi.="$Mf\n";else
echo"$Mf<td align='right'><label class='module'><span id='Rows-".h($C)."'></span>".checkbox("data[]",$C,$db,"","checkboxClick(event, this); formUncheck('check-data');")."</label>\n";$Jf[$If]++;}echo$oi;if($lh)echo"<script type='text/javascript'>ajaxSetHtml('".js_escape(ME)."script=db');</script>\n";}else{echo"<thead><tr><th style='text-align: left;'><label class='module'><input type='checkbox' id='check-databases'".($a==""?" checked":"")." onclick='formCheck(this, /^databases\\[/);'>".lang(35)."</label></thead>\n";$l=$b->databases();if($l){foreach($l
as$m){if(!information_schema($m)){$If=preg_replace('~_.*~','',$m);echo"<tr><td>".checkbox("databases[]",$m,$a==""||$a=="$If%",$m,"formUncheck('check-databases');","module")."\n";$Jf[$If]++;}}}else
...
echo"<td align='center'><label class='module'><input type='checkbox' name=$C value='1'".($Y?" checked":"").($Of=="All privileges"?" id='grants-$s-all'":($Of=="Grant option"?"":" onclick=\"if (this.checked) formUncheck('grants-$s-all');\""))."></label>";$s++;}}}echo"</table>\n",'<p>
<input type="submit" value="',lang(14),'">
