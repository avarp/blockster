...
class Controller extends \block\Controller
{
    ...
    public function __construct($view, $model)
    {
        ...
        if (empty($this->structure)) error404();
    ...
    public function actionIndex()
    {
        ...
        if (isset($_GET['type']) && isset($_GET['id'])) {
            ...
            if (empty($this->model->getRecordById($_GET['type'], $_GET['id'])) ||
                !isset($this->structure[$_GET['type']]['childs']) ||
                empty($this->structure[$_GET['type']]['childs'])) {
                error404();
            }

            $childs = $this->structure[$_GET['type']]['childs'];
            foreach ($childs as $sqlName) $tables[$sqlName] = $this->structure[$sqlName]['name'];
        } else {
            foreach ($this->structure as $sqlName => $table) if ($table['isRoot']) $tables[$sqlName] = $table['name'];
        }
        
        $breadcrumbs = array();

        $this->view->setTemplate('index.tpl');
        $this->view->linkJs('/blocks/backend/infobase/templates/script.js');
    ...
    public function actionShowTable($params)
    {
        ...
        * $params['selectedRecords'] - массив выделенных записей
        *
        * команды:
        * $_GET['deleteRecords'] - удалить записи
        * $_['pasteRecords'] - вставить записи
